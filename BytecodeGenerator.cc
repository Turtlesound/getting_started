...
void BytecodeGenerator::generateInstructionBytecode(const Instruction& instr) {
    switch(instr.type) {
        ...
        case Instruction::BINOP:
            ...
            // Store result (result of binop is on stack, then stored to instr.result variable)
            outputFile << "    istore " << getVarIndex(instr.result) << std::endl;
            break;
        ...
        case Instruction::GOTO:
            ...
            if (instr.result.rfind("block_", 0) == 0) { // Check if starts with "block_"
                std::string label = "L" + instr.result.substr(6);
                outputFile << "    goto " << label << std::endl;
            } else {
                outputFile << "    goto " << instr.result << std::endl;
            }
            break;
            
        case Instruction::IF_TRUE:
            {
                // Assuming instr.op1 is the variable name holding the boolean condition result
                // (e.g., the result of a comparison stored by a BINOP instruction's `istore getVarIndex(instr.result)`)
                // Load this variable onto the stack.
                outputFile << "    iload " << getVarIndex(instr.op1) << std::endl; 

                std::string labelStr = instr.result; // instr.result is the target label string
                if (labelStr.rfind("block_", 0) == 0) { // Check if starts with "block_"
...
void BytecodeGenerator::generateBytecode(const CFG& cfg, const std::string& outputFileName) {
    ...
    for (const auto& [className, methods] : methodsByClass) {
        ...
        for (const auto& [methodName, entryBlock] : methods) {
            ...
            // D1.main()I is treated as an instance method by invokestatic D1/main()I -> new D1().main() effectively
            // Or, if it were truly static, 'this' wouldn't be used.
            // For simplicity, assume 'this' is slot 0 if it's not the special static main.
            // The generated bytecode for D1.main()I doesn't use 'this', but Sum.calcSum does implicitly.
            ...
            // Example: For Sum.calcSum(num):
            // this -> 0, localVarCounter = 1
            // num  -> 1, localVarCounter = 2
            // Subsequent local variables like 'sum' will get indices from 2 onwards.

            int actualParamCount = currentMethodParamNames.size();
            std::string methodSignatureStr = "(";
            for (int i = 0; i < actualParamCount; i++) {
                methodSignatureStr += "I"; // Assuming all parameters are integers for now
            }
            // Assuming all methods in D1.java example return int for simplicity of this fix
            methodSignatureStr += ")I"; 
            
            methodSignatures[methodKey] = methodSignatureStr; // Store the correct signature
